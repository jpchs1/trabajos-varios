...
- **[Cotización Segar — San Pedro de Atacama](cotizaciones/segar-san-pedro-atacama/)** (COT-2026-0160) — Programa de cinco días en San Pedro, 22 al 26 de diciembre de 2026, para 4 pasajeros. Dos documentos, cada uno en español e inglés: la **cotización**, con el subtotal por tour (servicio + entradas) y el total del pack, y el **comparativo de días y horarios**, que cruza servicio por servicio la cotización contra el itinerario cargado en el sistema Tourevo y lista los choques que hay que destrabar antes de reservar. Los precios viven una sola vez en `contenido.mjs`; `node build.mjs` y `node build-diferencias.mjs` regeneran HTML y PDF en los dos idiomas. Va en `cotizaciones/` y no en `propuestas/` para no reactivar el árbol que se mudó a `tourevo-cl/website/`, que el deploy por FTP publica automáticamente.
