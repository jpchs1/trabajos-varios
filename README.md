...
- **[Cotización Segar — San Pedro de Atacama](cotizaciones/segar-san-pedro-atacama/)** (COT-2026-0160) — Programa de cinco días en San Pedro, en dos versiones completas: [español](cotizaciones/segar-san-pedro-atacama/index.html) e [inglés](cotizaciones/segar-san-pedro-atacama/index-en.html), más los dos PDF. Cada tour lleva su subtotal por persona (servicio + entradas) y el documento cierra con el total del pack. Los precios viven una sola vez en `contenido.mjs` y `node build.mjs` regenera HTML y PDF en los dos idiomas. Va en `cotizaciones/` y no en `propuestas/` para no reactivar el árbol que se mudó a `tourevo-cl/website/`, que el deploy por FTP publica automáticamente.
