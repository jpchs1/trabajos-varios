...
def P(before=0, after=0, line=1.16, keep=False, indent=None, hang=None, tabs=(), just=False):
...
def rail_bullet(label, before=16):
    ...
    p.paragraph_format.space_after = Pt(4.4)
    return p

def bullet(pre="", key="", post="", after=4.4):
...
def job(company, role, dates, first=False, label=None, before=10.5):
...
for txt in (
  ...
  "prevención del suicidio."):
    q = P(before=5, line=1.18, indent=RAIL, just=True); R(q, txt)
...
job("Fundación Fondo Esperanza", "Monitora · Grupo Emprendedores", "Octubre 2017 – Septiembre 2018", before=16)
