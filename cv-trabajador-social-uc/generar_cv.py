...
doc = Document()
s = doc.sections[0]
s.top_margin = Cm(1.3); s.bottom_margin = Cm(1.2)
...
def heading(text):
    p = doc.add_paragraph(); sp(p, before=10, after=4.5, keep=True)
...
def entry(title, inst, year):
    p = doc.add_paragraph(); sp(p, before=3.5, after=0, keep=True)
    p.paragraph_format.tab_stops.add_tab_stop(W, WD_TAB_ALIGNMENT.RIGHT)
...
run(p, "JENIFFER MIERES CONTRERAS", size=20, bold=True, color=NAVY, spacing=8)
...
run(p, "+56 9 4903 1682  ·  jeniffer.mieres@gmail.com  ·  El Monte, Región Metropolitana",
    size=9.5, color=MID)
...
run(p,
 "Trabajadora Social titulada con distinción y más de 9 años de experiencia continua en bienestar laboral, "
 "gestión de beneficios y atención social directa, desarrollada en instituciones de educación superior y en "
 "empresa privada. Actualmente en la Universidad de Santiago de Chile, donde realizo evaluaciones "
 "socioeconómicas, gestiono beneficios ministeriales e internos y acompaño casos que requieren articular áreas "
 "internas con redes de apoyo. Antes lideré la gestión de personas y bienestar en Bmining —programas, "
 "convenios, actividades y clima laboral— y encabecé el Departamento de Bienestar Social de una dotación "
 "superior a 200 colaboradores, incluida la administración del Seguro Complementario de Salud. A la atención "
 "directa sumo una mirada de proceso: fui auditora interna del Sistema de Gestión Integrado (ISO 9001, 14001 y "
 "45001), lo que me permite implementar y evaluar programas con estándares verificables. Diplomada en Bienestar "
 "Organizacional y Estrategias de Diversidad e Inclusión (USACH) y certificada como agente Gatekeeper en "
 "prevención del suicidio.", size=10)

# ---------------- ÁREAS DE EXPERTISE ----------------
heading("Áreas de expertise")
p = doc.add_paragraph(); sp(p, after=0, line=1.09)
run(p, "Diseño, implementación y evaluación de programas de bienestar  ·  Gestión de beneficios internos y externos  ·  "
       ...
       "Diversidad e inclusión  ·  Calidad de servicio  ·  Normativa laboral y gestión documental", size=9.5)

# ---------------- EXPERIENCIA ----------------
heading("Experiencia profesional")

job("Universidad de Santiago de Chile", "Agosto 2023 – Presente",
    "Trabajadora Social · Departamento de Beneficios Estudiantiles")
bullet("Realizo la ", "evaluación socioeconómica",
       " de cada caso —revisión de antecedentes y acreditaciones— para definir con autonomía qué apoyo "
       "corresponde, aplicando la normativa vigente.")
bullet("Gestiono ", "beneficios ministeriales e internos",
       " de principio a fin: verificación de requisitos, asignación, seguimiento y registro documental que "
...
bullet("Atiendo público de manera permanente y entrego ", "orientación social personalizada",
       ", acompañando al usuario durante todo el proceso de postulación.")
...
job("Bmining · Desarrollo e Innovación para la Minería SpA", "Octubre 2018 – Abril 2023",
...
bullet("Fui ", "contraparte directa de los trabajadores",
       ": levantaba cada necesidad y le buscaba solución con las jefaturas o derivando a la red que correspondía.")
bullet("Apoyé a Recursos Humanos en ", "selección e inducción", " y participé en las capacitaciones internas.")
...
bullet("Los ", "vinculé con redes y recursos",
       " disponibles que muchas veces no sabían que tenían a su alcance.")

job("Ferretería San Francisco", "Diciembre 2016 – Septiembre 2017",
    "Encargada · Departamento de Bienestar Social")
bullet("Encabecé el ", "área de bienestar social para una dotación superior a 200 colaboradores",
       ", planificando actividades y beneficios para los trabajadores.")
...
bullet("", "Certificación Gatekeepers",
       " — agentes comunitarios en prevención del suicidio (2026).")
...
bullet("", "Ley 21.327, Modernización de la Dirección del Trabajo",
       " — Bicentenario OTEC (2021).")
bullet("", "Cálculo de finiquito",
       " — Consultores y Asesores en Capacitación Chile Ltda. (2022).")

# ---------------- COMPETENCIAS ----------------
heading("Competencias y herramientas")
p = doc.add_paragraph(); sp(p, after=3, line=1.09)
...
bullet("", "Herramientas: ",
       "Microsoft Office (Word, Excel y PowerPoint) nivel intermedio; plataformas institucionales de registro "
